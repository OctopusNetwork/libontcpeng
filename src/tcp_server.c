...
#define ONC_MAX_TCP_SERVER_COUNT        32
...
typedef struct {
    ...
    tcp_listener_s_t   *servers;
    void               *evgrp;
    int                 max_servers;
    int                 cur_server_index;
} tcp_srvmod_s_t;

static tcp_srvmod_s_t   g_tcp_srvmod;

static tcp_listener_s_t *__alloc_listener(void)
{
    int i = 0;

    for (i = 0; i < g_tcp_srvmod.max_servers; i++) {
        tcp_listener_s_t *server = &g_tcp_srvmod.servers[i];
        if (0 == server->used) {
            server->used = 1;
            return server;
        }
...
static void __free_listener(tcp_listener_s_t *listener)
{
    listener->used = 0;
}

static tcp_listener_s_t *
__create_listener(onc_ip_t ip, onc_port_t port, int max_connections)
{
    void *tcp_sock = NULL;
    tcp_listener_s_t *listener = NULL;
    ...
    tcp_sock = tcp_socket_new(ip, port, max_connections,
            ONC_EVENT_READ | ONC_EVENT_WRITE | ONC_EVENT_ERROR);
    if (NULL == tcp_sock) {
...
static void __destroy_listener(tcp_listener_s_t *listener)
{
...
static void __destroy_listeners(void)
{
    ...
    for (i = 0; i < g_tcp_srvmod.max_servers; i++) {
        tcp_listener_s_t *listener = &g_tcp_srvmod.servers[i];
        if (1 == listener->used) {
...
{
    g_tcp_srvmod.servers = onc_malloc(
            max_servers * sizeof(tcp_listener_s_t));
    if (NULL == g_tcp_srvmod.servers) {
        return -1;
    }

    onc_memset(g_tcp_srvmod.servers, 0x0,
            max_servers * sizeof(tcp_listener_s_t));
    ...
    if (0 != internal_evgrp) {
        ...
        g_tcp_srvmod.evgrp = onc_evgrp_create(
                ONC_MAX_TCP_SERVER_COUNT);
        if (NULL == g_tcp_srvmod.evgrp) {
            onc_free(g_tcp_srvmod.servers);
            return -1;
        }
    } else {
        if (NULL == evgrp) {
...
void tcp_server_final(void)
{
    if (0 != g_tcp_srvmod.internal_evgrp) {
        onc_evgrp_destroy(g_tcp_srvmod.evgrp);
    }
    __destroy_listeners();
    onc_free(g_tcp_srvmod.servers);
}
...
{
    tcp_listener_s_t *tcp_listener = (tcp_listener_s_t *)listener;
...
void *tcp_server_accept(int millseconds)
{
    ...
    rc = onc_evgrp_wait(g_tcp_srvmod.evgrp, millseconds, lfds);
    if (rc < 0) {
        goto L_ERROR;
    } else if (0 < rc) {
        tcp_listener_s_t *listener = NULL;
